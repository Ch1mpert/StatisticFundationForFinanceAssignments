...
def inverse_fourier_transform(x, df, a=-3, b=3, n=1200):
...
def plot_distributions(df):
    ...
    plt.grid(True)
    plt.show()
    pctg_err = [100*abs((y - t))/t for y, t in zip(y_values, t_values)]
    plt.plot(x_values, pctg_err, label = 'Distance', color='green',linewidth=2)
    plt.title('Plot of the Percentage Error of the Estimated Distribution')
    plt.show()
    return

plot_distributions(30)
plot_distributions(3)
...
#Note that we cannot compare these as they have different CI

#To compare them, set the CI of nonparametric to the same as parametric by setting n=250, so choose 250/500 data points per bootstrap:
def nonparametric_bootstrap_CI250(data, ESlevel=0.05, B=500, CI=0.90, n=250):
    ES_samples = []
    for _ in range(B):
        bootstrap_sample = np.random.choice(data, n, replace=True)
        ES = expected_shortfall(bootstrap_sample, ESlevel)
...
def check_coverage_probability(true_ES, df, n=500, iterations=250):
    nonparametric_coverage = 0
    parametric_coverage = 0
    nonparametric_coverage250 = 0
    len_parametric = []
    len_nonparametric = []
    len_nonparametric250 = []
    for _ in tqdm(range(iterations)):
        data=stats.t.rvs(df, loc=0, scale=1.0, size=n)
        nonparametric_ES_CI_lower, nonparametric_ES_CI_upper, u = nonparametric_bootstrap_CI(data)
        nonparametric_ES_CI_lower250, nonparametric_ES_CI_upper250, z = nonparametric_bootstrap_CI250(data)
        parametric_ES_CI_lower, parametric_ES_CI_upper, v = parametric_bootstrap_CI(data)
        ...
        len_nonparametric250.append(nonparametric_ES_CI_upper250 - nonparametric_ES_CI_lower250)
        if nonparametric_ES_CI_lower <= true_ES <= nonparametric_ES_CI_upper:
            nonparametric_coverage += 1
        if nonparametric_ES_CI_lower250 <= true_ES <= nonparametric_ES_CI_upper250:
            nonparametric_coverage250 += 1
    ...
    nonparametric_coverage250_prob = nonparametric_coverage250 / iterations
    avg_len_parametric = mean(len_parametric)
    avg_len_nonparametric = mean(len_nonparametric)
    avg_len_nonparametric250 = mean(len_nonparametric250)
    return nonparametric_coverage_prob, parametric_coverage_prob, avg_len_parametric, avg_len_nonparametric, avg_len_nonparametric250, nonparametric_coverage250_prob
...
nonparametric_coverage_prob, parametric_coverage_prob, avg_len_parametric, avg_len_nonparametric, avg_len_nonparametric250, nonparametric_coverage250_prob = check_coverage_probability(true_ES, df1)

print(f"Nonparametric Bootstrap Coverage Probability: {nonparametric_coverage_prob:.4f}")
print(f"Nonparametric Bootstrap Coverage Probability with choosing 250 out of 500 data points: {nonparametric_coverage250_prob:.4f}")
...
print(f"Average Length of 95% CL Nonparametric Bootstrap CI: {avg_len_nonparametric250:.4f}")
print(f"Average Length of 95% CL Parametric Bootstrap CI: {avg_len_parametric:.4f}")
